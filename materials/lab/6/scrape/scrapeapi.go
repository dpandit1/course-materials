        ...
        for _, r := range regexes {
            if r.MatchString(path) {      
                ...
                if w != nil && track>0 {
                    ...
                    w.Write([]byte(`"`+(strconv.FormatInt(int64(track), 10))+`":  `))
...
func MainPage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<html><body><H1>This is the home page. Add the API end points like search, clear, reset or addsearch to see the effets.</H1></body>")
}
...
func FindFile(w http.ResponseWriter, r *http.Request) {
    ...
    if ok && len(q[0]) > 0 {
		...
		if n == false{
			fmt.Fprintf(w, "<html><body><H1> No match found </H1></body> ")
		}			
